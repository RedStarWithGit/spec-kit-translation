---
description: 基于可用的设计工件为功能生成可操作的、按依赖排序的 tasks.md。
handoffs:
  - label: 分析一致性
...
1. **设置**：从仓库根目录运行 `{SCRIPT}` 并解析 FEATURE_DIR 和 AVAILABLE_DOCS 列表。所有路径必须是绝对路径。对于像 "I'm Groot" 这样的参数中的单引号，使用转义语法：例如 'I'\''m Groot'（或如果可能则使用双引号："I'm Groot"）。
...
tasks.md 应该立即可执行——每个任务必须足够具体，以便 LLM 可以在没有其他上下文的情况下完成它。
...
3. **[P] 标记**：仅当任务可并行化（不同文件，对未完成任务无依赖）时才包含
...
2. **从契约**：
   ...
   - 如果请求了测试：每个契约 → 在该故事阶段中在实现之前进行契约测试任务 [P]
