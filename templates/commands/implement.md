...
2. **检查检查清单状态**（如果 FEATURE_DIR/checklists/ 存在）：
   ...
   - **如果任何检查清单不完整**：
     - 显示带有未完成项目计数的表
     - **停止**并询问："一些检查清单不完整。您是否仍要继续实现？（是/否）"
     - 在继续之前等待用户响应
     - 如果用户说 "否" 或 "等待" 或 "停止"，停止执行
     - 如果用户说 "是" 或 "继续" 或 "proceed"，继续执行步骤 3
...
6. 按照任务计划执行实现：
   - **按阶段执行**：在移动到下一个阶段之前完成每个阶段
   - **尊重依赖关系**：按顺序运行顺序任务，可以一起运行并行任务 [P]
   - **遵循 TDD 方法**：在相应的实现任务之前执行测试任务
   - **基于文件的协调**：影响同一文件的任务必须按顺序运行
   - **验证检查点**：在继续之前验证每个阶段完成

7. 实现执行规则：
   - **设置优先**：初始化项目结构、依赖项、配置
   - **测试优先代码**：如果您需要为契约、实体和集成场景编写测试
