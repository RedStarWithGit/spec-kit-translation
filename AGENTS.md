...
**GitHub Spec Kit** 是一个用于实现 Spec-Driven Development（SDD，即规范驱动开发）的综合工具包——这是一种强调在实施之前创建清晰规范的方法论。该工具包包含模板、脚本和工作流，指导开发团队通过结构化的方式构建软件。

**Specify CLI** 是命令行界面，用于使用 Spec Kit 框架引导项目。它设置了必要的目录结构、模板和 AI 代理集成，以支持规范驱动开发工作流。

该工具包支持多种 AI 编程助手，允许团队使用其首选工具，同时保持一致的项目结构和开发实践。

---

## 一般实践

- 对 Specify CLI 的 `__init__.py` 的任何更改都需要在 `pyproject.toml` 中升级版本并在 `CHANGELOG.md` 中添加条目。

## 添加新代理支持

本节说明如何向 Specify CLI 添加对新 AI 代理/助手的支持。在将新 AI 工具集成到规范驱动开发工作流时，请参考本指南。
...
按照这些步骤添加新代理（以假设的新代理为例）：

#### 1. 添加到 AGENT_CONFIG

**重要提示**：使用实际的 CLI 工具名称作为键，而不是缩写版本。

将新代理添加到 `src/specify_cli/__init__.py` 中的 `AGENT_CONFIG` 字典。这是所有代理元数据的**唯一真实来源**：
...
AGENT_CONFIG = {
    ...
    "new-agent-cli": {  # 使用实际的 CLI 工具名称（用户在终端中输入的名称）
        ...
        "install_url": "https://example.com/install",  # 安装文档 URL（如果是基于 IDE 的代理则为 None）
        "requires_cli": True,  # 如果需要 CLI 工具则为 True，基于 IDE 的代理则为 False
    },
}
```

**关键设计原则**：字典键应匹配用户安装的实际可执行文件名。例如：

- ✅ 使用 `"cursor-agent"`，因为 CLI 工具确实叫 `cursor-agent`
- ❌ 如果工具是 `cursor-agent`，不要使用 `"cursor"` 作为缩写

这消除了在整个代码库中需要特殊情况映射的需求。
...
- `requires_cli`：代理是否需要在初始化期间检查 CLI 工具

#### 2. 更新 CLI 帮助文本

在 `init()` 命令中更新 `--ai` 参数帮助文本以包含新代理：

```python
ai_assistant: str = typer.Option(None, "--ai", help="AI assistant to use: claude, gemini, copilot, cursor-agent, qwen, opencode, codex, windsurf, kilocode, auggie, codebuddy, new-agent-cli, or q"),
```

同时更新任何列出可用代理的函数文档字符串、示例和错误消息。

#### 3. 更新 README 文档

更新 `README.md` 中的**支持的 AI 代理**部分以包含新代理：

- 在表格中添加新代理及适当的支持级别（完整/部分）
- 包含代理的官方网站链接
- 添加关于代理实施的任何相关说明
- 确保表格格式保持对齐和一致
...
**关键**：在向 AGENT_CONFIG 添加新代理时，始终使用**实际的可执行文件名**作为字典键，而不是缩写或便捷版本。

**为什么这很重要**：

- `check_tool()` 函数使用 `shutil.which(tool)` 在系统 PATH 中查找可执行文件
- 如果键不匹配实际的 CLI 工具名称，您将需要在整个代码库中使用特殊情况的映射
- 这会创建不必要的复杂性和维护负担
...
```python
AGENT_CONFIG = {
    "cursor-agent": {  # 匹配实际的可执行文件名
        "name": "Cursor",
        # ...
    }
}

# 不需要特殊情况——直接使用 agent_key！
```
...
对于可用作 VS Code 扩展的代理，将它们添加到 `.devcontainer/devcontainer.json`：
...
- **基于扩展的代理**：添加到 `devcontainer.json` 的 `extensions` 数组
- **基于 CLI 的代理**：添加安装脚本到 `post-create.sh`
- **混合代理**：可能需要扩展和 CLI 安装
...
- **GitHub Copilot**：内置在 VS Code/兼容编辑器中
- **Windsurf**：内置在 Windsurf IDE 中
- **IBM Bob**：内置在 IBM Bob IDE 中

## 命令文件格式

### Markdown 格式

使用于：Claude、Cursor、opencode、Windsurf、Amazon Q Developer、Amp、SHAI、IBM Bob
...
1. **使用缩写键而不是实际的 CLI 工具名称**：始终使用实际的可执行文件名作为 AGENT_CONFIG 键（例如，`"cursor-agent"` 而不是 `"cursor"`）。这防止在整个代码库中需要特殊情况的映射。
2. **忘记更新脚本**：添加新代理时必须同时更新 bash 和 PowerShell 脚本。
3. **不正确的 `requires_cli` 值**：仅对实际有 CLI 工具要检查的代理设置为 `True`；对于基于 IDE 的代理设置为 `False`。
4. **错误的参数格式**：对每种代理类型使用正确的占位符格式（Markdown 用 `$ARGUMENTS`，TOML 用 `{{args}}`）。
